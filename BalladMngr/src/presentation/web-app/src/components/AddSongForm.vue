    ...
    <v-row justify="center">
        <v-dialog v-model="dialog" persistent max-width="600px">
            <template v-slot:activator="{ on, attrs }">
                <v-btn style="margin-top: 1rem" rounded color="light-blue" dark v-bind="attrs" v-on="on">
                    <v-icon left>mdi-plus</v-icon>
                    Koleksiyona yeni bir beste ekle.
                </v-btn>
            </template>
            <v-card>
                <form @submit.prevent="addSongAction(body); body = {};">
                    <v-card-title>
                    ...
                    <v-card-text>
                        <v-container>
                            <v-row>
                                <v-col cols="12" sm="6">
                                    <v-text-field label="Adı" v-model="body.title" @input="$v.body.title.$touch()"
                                        @blur="$v.body.title.$touch()" :error-messages="titleErrors" required>
                                    </v-text-field>
                                </v-col>
                                <v-col cols="12">
                                    <v-textarea label="Sözleri" v-model="body.lyrics" @input="$v.body.lyrics.$touch()"
                                        @blur="$v.body.lyrics.$touch()" :error-messages="lyricErrors" required>
                                    </v-textarea>
                                </v-col>

                                <v-col cols="12" sm="6">
                                    <v-select v-model="body.language" :items="languages" item-text="name"
                                        item-value="id" label="Dili" persistent-hint return-object single-line>
                                    </v-select>
                                </v-col>
                            </v-row>
                        </v-container>
                    ...
                    <v-card-actions>
                        ...
                        <v-btn color="blue darken-1" text @click="dialog = false" :disabled="$v.$anyError"
                            type="submit">
                            Save
                        </v-btn>
                    </v-card-actions>
